    ...
    <main className="playground min-h-screen pb-16">
      <div className="mx-auto max-w-2xl px-5 pt-8 sm:px-8">
        ...
        <section className="mt-6">
          ...
          <ul className="mt-2.5 space-y-2 rounded-2xl border border-grape/15 bg-grape/5 p-4">
            {review.keyPoints.map((k, i) => (
              <li key={i} className="flex items-start gap-2.5 text-sm leading-relaxed text-ink">
                <span className="mt-0.5 shrink-0 text-grape-deep">●</span>
                <Rich text={k} />
              </li>
            ))}
          </ul>
          ...
          <section className="mt-6">
            ...
            <div className="mt-2.5 space-y-3">
              {review.commonMistakes.map((m, i) => (
                <div key={i} className="rounded-2xl border border-tomato/20 bg-white p-4 shadow-card">
                  <p className="text-sm font-medium text-tomato-deep">❌ {m.mistake}</p>
        ...
        {review.tips.length > 0 && (
          <section className="mt-6">
            ...
            <ul className="mt-2.5 space-y-2 rounded-2xl border border-sun/25 bg-sun/5 p-4">
              {review.tips.map((t, i) => (
                <li key={i} className="flex items-start gap-2.5 text-sm leading-relaxed text-ink">
                  <span className="mt-0.5 shrink-0 text-sun-deep">✦</span>
                  <Rich text={t} />
                </li>
              ))}
            </ul>
                ...
                return (
                  <button
                    ...
                    className={`min-h-[110px] rounded-2xl border-2 p-4 text-left shadow-card transition ${
                      isFlipped ? "border-mint bg-mint/5" : "border-grape/25 bg-white hover:border-grape/50"
                    }`}
                  >
                    <span
                      className={`font-mono text-[10px] font-bold uppercase tracking-wider ${
                        isFlipped ? "text-mint-deep" : "text-grape-deep"
                      }`}
                    >
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-center gap-3 rounded-xl border p-3.5 text-left text-sm transition ${
                      isChecked ? "border-mint/40 bg-mint/10 text-ink" : "border-ink/10 bg-white text-ink-soft"
                    }`}
                  >
                    <span
                      className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-md border-2 text-xs font-bold ${
                        isChecked ? "border-mint bg-mint text-white" : "border-ink/20 text-transparent"
                      }`}
                    >
                      ✓
                    </span>
